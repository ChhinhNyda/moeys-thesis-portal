...
export default function App() {
  ...
  const [heiContext, setHeiContext] = useState("RUPP"); // which HEI is the user from, when role=hei
  const [theses, setTheses] = useState([]);
  const [heis, setHeis] = useState(DEFAULT_HEIS);
  ...
  useEffect(() => {
    (async () => {
      const [t, h, r] = await Promise.all([
        sGet(STORAGE_THESES, SAMPLE_THESES),
        sGet(STORAGE_HEIS, DEFAULT_HEIS),
        sGet(STORAGE_ROLE, { role: "public", heiContext: "RUPP" }),
      ]);
      setTheses(t); setHeis(h);
